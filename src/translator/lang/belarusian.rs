...
impl Translator for Belarusian {
  ...
  mod tests {
    ...
    fn test_translator_lang_belarusian_to_latin() {
      //Simple commands
      let translator: Box<dyn Translator> = new_translator(Language::Belarusian);
      //ls -l
      ...
      println!("\"{}\" => \"{}\"", input, output);
      assert_eq!(output, "K O");
      //Special case: k as last character, but preceeded by 'к' | 'а' | 'і' | 'о'
